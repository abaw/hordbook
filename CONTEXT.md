...
- **Prompt action** — a button on a word card that composes a structured prompt for ChatGPT and opens it via the `chatgpt.com/?q=…` deep link, with the same text copied to the clipboard as a fallback. Three in v1: *Example sentences*, *Teach me inside out*, *Compare with similar words*. Output is always English. Custom GPT links cannot be prefilled, so the app does not use one (ADR-0001).
