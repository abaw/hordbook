...
describe("App root", () => {
  it("renders the Home screen with the collection it was given", async () => {
    await renderApp({ collection: fixtureCollection({ name: "Fixture Collection 0.1", wordCount: 3 }) });
  ...
  it("shows the Add to Home Screen hint until it is dismissed, and remembers the dismissal", async () => {
    ...
    const first = await renderApp({ progressStore });
  ...
  it("does not show the hint when already running as an installed app", async () => {
    await renderApp({ platform: fakePlatform({ isStandalone: true }) });
    await screen.findByRole("heading", { level: 1 });
    expect(screen.queryByRole("note", { name: /add to home screen/i })).not.toBeInTheDocument();
  });
});
